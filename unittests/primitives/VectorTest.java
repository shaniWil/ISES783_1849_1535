...
class VectorTest {
    ...
    public void testScale() {
        ...
        assertEquals(new Vector(2, 4, 6),
                v1.scale(2),
                "The Vector multiplication by scalar failed");
    ...
    public void testDotProduct() {
        ...
        assertEquals(-28d,
                d2,
                0.00001,
                "dotProduct() wrong result length");
        ...
        assertFalse(isZero(v1.dotProduct(new Vector(1, 3, -2))),
                "dotProduct() result is not orthogonal to 1st operand");

        assertFalse(isZero(v1.dotProduct(new Vector(0, 2, -2))),
                "dotProduct() result is not orthogonal to 2nd operand");

        assertFalse(isZero(v1.dotProduct(new Vector(0, 3, 0))),
                "dotProduct() result is not orthogonal to 3rd operand");
    }
    ...
    public void testCrossProduct() {
        ...
        assertThrows(IllegalArgumentException.class,
                ...
                "ERROR: crossProduct() for parallel vectors");

        //TC11: Test cross product by length
        assertEquals(v1.length() * v2.length(),
                v1.crossProduct(v2).length(),
                0.00001,
                "ERROR: crossProduct() wrong result length");

        //TC12: Test that the result Vector orthogonal to 2 original vectors
        assertEquals(0d,
                v1.crossProduct(v2).dotProduct(v1),
                "ERROR: crossProduct() result is not orthogonal to its 1st operand");

        assertEquals(0d,
                v1.crossProduct(v2).dotProduct(v2),
                "ERROR: crossProduct() result is not orthogonal to its 2nd operand");
    }
    ...
    public void testLength() {
        ...
        assertEquals(3d,
                v4.length(),
                0.00001d,
                "ERROR: length() result is incorrect");
    }
    ...
    public void testNormalize() {
        ...
        assertEquals(1d,
                v1.normalize().length(),
                0.00001d,
                "ERROR: normalize() the Vector length should be equal to 1");
    }
}